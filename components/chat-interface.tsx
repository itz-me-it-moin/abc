...
export default function ChatInterface() {
  ...
  // Generate gradient colors based on current hue
  const generateGradient = () => {
    const hue1 = hue
    const hue2 = (hue + 60) % 360 // 60 degrees offset for complementary color
    return {
      background: `linear-gradient(45deg, hsl(${hue1}, 100%, 50%), hsl(${hue2}, 100%, 50%))`,
    }
  }

  // Generate glow effect based on audio level
  const generateGlow = () => {
    const intensity = 10 + audioLevel * 30
    const opacity = 0.3 + audioLevel * 0.5
    return {
      boxShadow: `0 0 ${intensity}px ${intensity / 2}px hsla(${hue}, 100%, 50%, ${opacity})`,
    }
  }
